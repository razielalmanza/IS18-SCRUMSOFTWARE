...
@NamedNativeQueries(value = { @NamedNativeQuery( name = "RespuestaConexionBD.canRespuestaConexionBD", query = "select modeloforo.verificar(?, ?)" ),
							  @NamedNativeQuery( name = "RespuestaConexionBD.findByUsuarioAndPassword",
												 query = "SELECT idUsuario, nombreUsuario FROM modeloforo.usuario WHERE nombreusuario = ?1 AND contrasena = crypt(?2, contrasena)",
												 resultClass = RespuestaConexionBD.class ) } )
