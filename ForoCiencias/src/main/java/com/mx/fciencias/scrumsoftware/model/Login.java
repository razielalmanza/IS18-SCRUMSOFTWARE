...
@NamedNativeQueries(value = {
    @NamedNativeQuery(
            name = "Login.canLogin",
            query = "select modeloforo.verificar(?, ?)"
    )
    , @NamedNativeQuery(
            name = "Login.findByUsuarioAndPassword",
            query = "select idusuario as id, nombreusuario as usuario from modeloforo.usuario where nombreusuario = ?1 and contrasena = crypt(?2, contrasena)",
